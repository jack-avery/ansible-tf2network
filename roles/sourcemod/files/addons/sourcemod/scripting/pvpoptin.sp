...
void UpdateActiveState(eGameState gameState) {
	...
	if (isActive != wasActive) {
		if (isActive) {
			MC_PrintToChatAll("%t", "Plugin now active");
			DHooksAttach();
		} else {
			MC_PrintToChatAll("%t", "Plugin now inactive");
			DHooksDetach();
		}
		for (int i=1;i<MaxClients;i++) {
			if (IsClientInGame(i))
...
public Action Command_TogglePvP(int client, int args) {
	if (clientPvPBannedUntil[client] > GetTime()) {
		MC_PrintToChat(client, "%t", "You have been banned from pvp", RoundToCeil((clientPvPBannedUntil[client]-GetTime())/60.0), clientPvPBannedReason[client]);
		return Plugin_Handled;
	}
	if (GetCmdArgs()==0) {
		char buffer[32];
...
public int HandlePickPlayerMenu(Menu menu, MenuAction action, int param1, int param2) {
	if (action == MenuAction_End) {
		...
		if (!IsClientInGame(target)) {
			MC_PrintToChat(param1, "%t", "Player no longer available");
...
public Action Command_ForcePvP(int client, int args) {
	if (GetCmdArgs()!=2) {
		...
		if (StrEqual(pattern, "map", false)) {
			...
			if (pvpon) {
				MC_PrintToChatAll("%t", "Someone forced map pvp", client);
				MC_ReplyToCommand(client, "%t", "You forced map pvp");
			} else {
				MC_PrintToChatAll("%t", "Someone reset map pvp", client);
				MC_ReplyToCommand(client, "%t", "You reset map pvp");
			}
		} else {
			...
			if (matches < 1) {
				...
				for (int i;i<matches;i++) {
					...
					if (pvpon) {
						...
						MC_PrintToChat(player, "%t","Someone forced your global pvp", client);
					} else {
						globalPvP[player] &=~ State_Forced;
						MC_PrintToChat(player, "%t","Someone reset your global pvp", client);
					}
					UpdateEntityFlagsGlobalPvP(player, IsGlobalPvP(player));
				}
				if (pvpon) {
					MC_ReplyToCommand(client, "%t", "You forced someones global pvp", tname);
				} else {
					MC_ReplyToCommand(client, "%t", "You reset someones global pvp", tname);
				}
			}
		}
	}
		...
		if (matches < 1) {
			...
			for (int i;i<matches;i++) {
				...
				if (force) {
					...
					MC_PrintToChat(player, "%t","Someone forced your mirror damage", client);
				} else {
					mirrorDamage[player] &=~ State_Forced;
					MC_PrintToChat(player, "%t","Someone reset your mirror damage", client);
				}
			}
			if (force) {
				MC_ReplyToCommand(client, "%t", "You forced someones mirror damage", tname);
			} else {
				MC_ReplyToCommand(client, "%t", "You reset someones mirror damage", tname);
			}
		}
	}
	return Plugin_Handled;
...
public Action Command_StopPvP(int client, int args) {
	...
	if (primary) {
		MC_PrintToChat(client, "%t", "Use command again to toggle ignore");
	} else {
...
static void RequestPairPvP(int requester, int requestee, bool antiSpam=false) {
	...
	if (requester == requestee || pairPvPignored[requestee] || clientPvPBannedUntil[requestee] > GetTime()) {
		...
		MC_PrintToChat(requester, "%t", "You have been banned from pvp", RoundToCeil((clientPvPBannedUntil[requester]-GetTime())/60.0), clientPvPBannedReason[requester]);
	} else if (antiSpam && (tmp = (PvP_PAIRREQUEST_COOLDOWN - (GetClientTime(requester) - clientLatestPvPRequest[requester]))) > 0.0) {
		MC_PrintToChat(requester, "%t", "Last pair pvp request too recent", RoundToCeil(tmp));
	} else if (IsFakeClient(requestee)) {
		MC_PrintToChat(requester, "%t", "Bots can not use pair pvp");
	} else if (pairPvP[requester][requestee]) { //already paired, leave
		MC_PrintToChat(requestee, "%t", "Someone disengaged pair pvp", requester);
		MC_PrintToChat(requester, "%t", "You disengaged pair pvp", requestee);
		...
		MC_PrintToChat(requestee, "%t", "You engaged pair pvp", requester);
		MC_PrintToChat(requester, "%t", "You engaged pair pvp", requestee);
		pairPvPrequest[requester]=pairPvPrequest[requestee]=0;
		SetPairPvP(requester,requestee,true);
	} else if (IsGlobalPvP(requester) && IsGlobalPvP(requestee)) {
		MC_PrintToChat(requester, "%t", "You are both global pvp");
	} else if (pairPvPrequest[requester]==requestee) {
		MC_PrintToChat(requester, "%t", "Already requested pvp with", requestee);
	} else if (pairPvPRequestMenu && ArrayFind(requestee, pairPvPrequest, sizeof(pairPvPrequest))>0) {
		//menus are kinda iffy
		MC_PrintToChat(requester, "%t", "There is a pending request with menus");
	} else {
		if (Notify_OnPairInvited(requester, requestee)) {
			if (IsValidClient(pairPvPrequest[requester])) {
				MC_PrintToChat(pairPvPrequest[requester], "%t", "Someone cancelled pvp request for another", requester);
				MC_PrintToChat(requester, "%t", "You cancelled pvp request", pairPvPrequest[requester]);
			}
			MC_PrintToChat(requestee, "%t", "Someone requested pvp, confirm", requester, requester);
			MC_PrintToChat(requester, "%t", "You requested pvp", requestee);
...
static void DeclinePairPvP(int requestee, bool CloseMenus=true) {
	...
	for (int requester=1; requester<=MaxClients; requester++) {
		if (IsValidClient(requester) && pairPvPrequest[requester]==requestee) {
			MC_PrintToChat(requester, "%t", "Your pvp request was declined", requestee);
	...
	if (declined>1) {
		MC_PrintToChat(requestee, "%t", "You declined multiple pvp requests", declined);
	} else if (declined==1) {
		MC_PrintToChat(requestee, "%t", "You declined single pvp request", someRequester);
	}
}
static void EndAllPairPvPFor(int client) {
	for (int i=1;i<=MaxClients;i++) {
		if (pairPvP[client][i]) {
			MC_PrintToChat(i, "%t", "Someone disengaged pair pvp", client);
		}
	}
	SetPairPvPClient(client, false);
	MC_PrintToChat(client, "%t", "You disengaged all pair pvp");
}
...
void PrintGlobalPvpState(int client) {
	...
	if (IsGlobalPvP(client)) {
		MC_PrintToChat(client, "%t", "Global pvp state on line1");
		MC_PrintToChat(client, "%t", "Global pvp state on line2");
	} else {
		MC_PrintToChat(client, "%t", "Global pvp state off line1");
		MC_PrintToChat(client, "%t", "Global pvp state off line2");
	}
	MC_PrintToChat(client, "%t", "Hey there's also pair pvp");
}
//return false if cancelled
bool SetGlobalPvP(int client, bool pvp, bool checkCooldown=false) {
	bool modelValid = IsPlayerModelValid(client);
	if (pvp && !modelValid) {
		MC_PrintToChat(client, "%t", "Model invalid for global pvp");
		return false;
	}
	...
	if (checkCooldown) {
		...
		if (!enterPvP && timeLeft > 0.0) {
			MC_PrintToChat(client, "%t", "Entered global pvp too recently", RoundToCeil(timeLeft));
			return false;
		}
...
static void SetPairPvPIgnored(int client, bool ignore) {
	...
	if (ignore) {
		DeclinePairPvP(client);
		MC_PrintToChat(client, "%t", "You are ignoring pair pvp");
	} else {
		MC_PrintToChat(client, "%t", "You are allowing pair pvp");
	}
}
//return false if cancelled
...
static void SetMirroredState(int client, bool mirrored) {
	...
	if (mirrored) MC_PrintToChat(client, "%t", "Mirror Damage Enabled");
	else if (IsMirrored(client)) MC_PrintToChat(client, "%t", "Mirror Damage not Disabled, Forced");
	else MC_PrintToChat(client, "%t", "Mirror Damage Disabled");
}
static void SetTauntKillable(int client, bool enabled) {
	...
	if (enabled) MC_PrintToChat(client, "%t", "Taunt Kills Enabled");
	else MC_PrintToChat(client, "%t", "Taunt Kills Disabled");
}
static void SetLimitedConditionsAllowed(int client, bool enabled) {
	...
	if (enabled) MC_PrintToChat(client, "%t", "Limited Conditions Enabled");
	else MC_PrintToChat(client, "%t", "Limited Conditions Disabled");
}
...
public void OnClientTakeDamagePost(int victim, int attacker, int inflictor, float damage, int damagetype) {
	...
	if (spawnKill_maxTime > 0.01 && spawnKill_minIncrease > 0 && spawnKill_maxIncreaseRoot >= 0.0 && spawnKill_threashold > 0 && spawnKill_banTime > 0) {
		...
		if (clientSpawnKillScore[attacker] >= spawnKill_threashold) { //5 near instant kills
			...
			MC_PrintToChat(attacker, "%t", "Spawn Killing is not allowed");
