...
public Action OnRoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	else
	{
		...
		if(GetConVarFloat(g_PublicTimer) == 1)
		{
			if( GetClientCount() != 0)
			{
				MC_PrintToChatAll("{cyan}[ {gray}NTM {cyan}] {red}ALERT: {orange}Round timer disabled");
			}
			else
			{
...
public Action NoTimeLimit_Enable(client, args)
{
	...
	if(args == 0 && !IsFakeClient(client) && IsClientInGame(client))
	{
		...
		if(client != 0)
		{
			MC_PrintToChat(client, "{cyan}[ {gray}NTM {cyan}] {orange}Time limit removed, if you want to re enable it, use {red}/ntm_off");
		}
		else
		{
...
public Action NoTimeLimit_Disable(client, args)
{
	...
	if(args == 0 && !IsFakeClient(client) && IsClientInGame(client))
	{
		...
		if(client != 0)
		{
			MC_PrintToChat(client, "{cyan}[ {gray}NTM {cyan}] {orange}Time limit restored, if you want to disable it, use {red}/ntm_on");
		}
		else
		{
	...
	if(GetConVarFloat(g_PublicTimer) == 1)
	{
		if( GetClientCount() != 0)
		{
			MC_PrintToChatAll("{cyan}[ {gray}NTM {cyan}] {red}ALERT: {orange}Round timer enabled");
...
public Action NoTimeLimit_SetTime(client, args)
{
	...
	if(args != 0 && !IsFakeClient(client) && IsClientInGame(client))
	{
		...
		if(client != 0)
		{
			MC_PrintToChat(client, "{cyan}[ {gray}NTM {cyan}] {orange}Time limit set to %s", settimeval);
		}
		else
		{
	...
	if(GetConVarFloat(g_PublicTimer) == 1)
	{
		if( GetClientCount() != 0)
		{
			MC_PrintToChatAll("{cyan}[ {gray}NTM {cyan}] {red}ALERT: {orange}Round timer set to %s", settimeval);
	...
	for(new i = 1; i <= MaxClients; i++)
		{
			if(IsClientInGame(i) && !IsFakeClient(i))
			{
				...
				{
					MC_PrintToChat(i, "{cyan}[ {gray}NTM {cyan}] {orange}Server using no time limit plugin by {red}ElPapuh");
				}
				if(GetConVarFloat(g_DisplayAnnounce) != 1)
				{
