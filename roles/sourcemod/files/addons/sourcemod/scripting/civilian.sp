...
public Action Command_Civil(int client, int args) // codigo dos comandos
{
	if(client == 0)
		{
			MC_ReplyToCommand(client, "{unique}[Civ]{default} %t", "IsConsole");
			return Plugin_Handled;
		}
	if(!IsPlayerAlive(client))
		{
		MC_ReplyToCommand(client, "{unique}[Civ]{default} %t", "IsDead")
		return Plugin_Handled
		}
	MC_ReplyToCommand(client, "{unique}[Civ]{default} %t", "BecameCiv");
...
public Action Command_ChangeStatus(int client, int args)
{
	...
	if(g_bCivAlways[client])
	{
		...
		MC_ReplyToCommand(client, "{unique}[Civ]{default} %t", "AlwaysCivE");
	}
	else
	{
		MC_ReplyToCommand(client, "{unique}[Civ]{default} %t", "AlwaysCivD");
	}
	return Plugin_Handled;
}
